...
template <typename T> class ListaE {
    public:
        ...
        explicit ListaE(int TamaMax); // constructor
        ListaE(const ListaE<T>& l); // ctor. de copia
        ListaE<T>& operator =(const ListaE<T>& l); // asignación entre listas
...
template <typename T> inline ListaE<T>::ListaE(int TamaMax) : elementos(new T[TamaMax]), Lmax(TamaMax), n(0) {}
